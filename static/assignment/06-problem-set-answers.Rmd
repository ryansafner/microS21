...
Malwart is a large retail chain that has opened in a small town. With few other job opportunities for low-skilled workers, it has monopsony power in the local labor market for low-skilled workers. Its marginal revenue product of labor is
$$MRP_L=80-0.8L$$
...
```{r}
mrp_l=function(q){80-0.8*q}
supply_l=function(q){0.4*q+8}
MC_l=function(q){0.8*q+8}
dwl_l<-tribble(
  ~x, ~y,
  45, 26,
  45, 44,
...
mon_i<-ggplot(data.frame(x=c(0,10)), aes(x=x))+
  geom_polygon(data=dwl_l, aes(x=x,y=y), fill="black", alpha=0.5)+
    stat_function(fun=MC_l, geom="line", size=2, color = "darkred")+
  geom_label(aes(x=80,y=MC_l(80)), color = "darkred", label="MC(L)", size = 3)+
    stat_function(fun=supply_l, geom="line", size=2, color = "red")+
  geom_label(aes(x=80,y=supply_l(80)), color = "red", label="Supply(L)", size = 3)+
  stat_function(fun=mrp_l, geom="line", size=2, color = "blue")+
  geom_label(aes(x=80,y=mrp_l(80)), color = "blue", label=expression(MRP[L]), size = 3)+
  ...
  geom_segment(x = 0, xend = 45, y = 26, yend = 26, linetype = "dotted", size = 0.5)+
  geom_segment(x = 45, xend = 45, y = 26, yend = 0, linetype = "dotted", size = 0.5)+
    geom_segment(x = 0, xend = 60, y = 32, yend = 32, linetype = "dotted", size = 0.5)+
  geom_segment(x = 60, xend = 60, y = 32, yend = 0, linetype = "dotted", size = 0.5)+

      geom_label(aes(x=45,y=10), label=expression(L[m]), size = 3)+
    geom_label(aes(x=60,y=10), label=expression(L[C]), size = 3)+

        geom_label(aes(x=10,y=26), label=expression(w[m]), size = 3)+
    geom_label(aes(x=10,y=32), label=expression(w[C]), size = 3)+
...
$$
\begin{align*}
\epsilon_{s}&= \frac{1}{slope} \times \frac{p}{q}\\
&= \frac{1}{0.4} \times \frac{26}{45} \\
&\approx 1.44 \\
\end{align*}
$$
...
First, update the new $MC(L)$ equation by doubling the slope of the labor supply curve to get 
...
```{r}
mrp_l=function(q){80-0.8*q}
supply_l=function(q){0.2*q+20}
MC_l=function(q){0.4*q+20}
dwl_l<-tribble(
  ~x, ~y,
  50, 30,
  50, 40,
...
mon_e<-ggplot(data.frame(x=c(0,10)), aes(x=x))+
  geom_polygon(data=dwl_l, aes(x=x,y=y), fill="black", alpha=0.5)+
    stat_function(fun=MC_l, geom="line", size=2, color = "darkred")+
  geom_label(aes(x=80,y=MC_l(80)), color = "darkred", label="MC(L)", size = 3)+
    stat_function(fun=supply_l, geom="line", size=2, color = "red")+
  ...
  geom_segment(aes(x = 0, xend = 30, y = 500, yend=500), linetype="dotted", size=1)+
    ...
    geom_segment(x = 0, xend = 50, y = 30, yend = 30, linetype = "dotted", size = 0.5)+
  geom_segment(x = 50, xend = 50, y = 30, yend = 0, linetype = "dotted", size = 0.5)+
    geom_segment(x = 0, xend = 60, y = 32, yend = 32, linetype = "dotted", size = 0.5)+
  geom_segment(x = 60, xend = 60, y = 32, yend = 0, linetype = "dotted", size = 0.5)+

    geom_label(aes(x=50,y=10), label=expression(L[m]), size = 3)+
    geom_label(aes(x=60,y=10), label=expression(L[C]), size = 3)+

        geom_label(aes(x=10,y=30), label=expression(w[m]), size = 3)+
    geom_label(aes(x=20,y=32), label=expression(w[C]), size = 3)+
...
The more elastic the labor supply curve (as in parts E & F), the less monopsony power (i.e. the more workers hired, the higher the wage, and the less DWL), as opposed to a less elastic labor supply curve (as in parts C & D).

```{r}
mon_e<-mon_e+ggtitle("More Elastic Supply")
mon_i<-mon_i+ggtitle("Less Elastic Supply")
library(patchwork)
mon_i + mon_e
```